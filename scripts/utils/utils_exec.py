...
def execute_all_steps(country_code, download=True, graph=True, skipdownload=""):
    ...
    overpass_extractor_folder = scriptfolder / "overpass_extractor"
    ...
    if download:
        if 'a' not in skipdownload:
            ...
            runpy.run_path(overpass_extractor_folder / "layerbuilder_countryshape.py", run_name="__main__")
        if 'b' not in skipdownload:
            print(">> Step1b - Download line and nodes")
            runpy.run_path(overpass_extractor_folder / "layerbuilder_linesxnodes.py", run_name="__main__")
        if 'c' not in skipdownload:
            print(">> Step1c - Download substations")
            runpy.run_path(overpass_extractor_folder / "layerbuilder_substations.py", run_name="__main__")
        if 'd' not in skipdownload:
            print(">> Step1d - Download circuit")
            runpy.run_path(overpass_extractor_folder / "layerbuilder_circuits.py", run_name="__main__")

    if graph:
        print(">> Step1 - Prepare for graph (pre-graph)")
        runpy.run_path(scriptfolder / "step1_prepare_for_graph.py", run_name="__main__")
        print(">> Step2 - Integrate circuit in pre-graph.py")
        runpy.run_path(scriptfolder / "step2_manage_circuit.py", run_name="__main__")
        print(">> Step3 - Build graph and export post-graph files")
